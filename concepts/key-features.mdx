---
title: "Key Features and Capabilities of mFoundry"
...
Every capability available in the mFoundry dashboard is also accessible programmatically through the mFoundry REST API. Authenticate with a token-based API key, then create projects, trigger workflow runs, query task status, and manage members — all from your own code or CI/CD pipelines.
...
<Tip>
  Want a feature that isn't listed here? mFoundry ships updates regularly based on customer feedback. Visit the **mFoundry Roadmap** from the help menu in your dashboard to upvote existing requests or submit a new one — the product team reviews submissions every sprint.
</Tip>
